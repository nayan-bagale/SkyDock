...
export const FilesExplorerCard = forwardRef<
  HTMLDivElement,
  FilesExplorerCardProps
>(
  (
    { style, onMouseDown, Action, children, title = "CatX", navButtons },
    ref,
  ) => {
    return (
      <AnimatePresence>
        <motion.div
          className={cn(
            " text-black resize shadow absolute w-[36rem] h-[22rem] min-w-[36rem] max-w-[55rem] min-h-[18rem] max-h-[40rem] z-20 bg-white/80 backdrop-blur rounded-xl overflow-hidden",
          )}
          ref={ref}
        ...
        >
          <div
            className=" flex items-center shadow relative justify-between px-2 py-1 w-full bg-slate-200/60 rounded"
            onMouseDown={onMouseDown}
          >
            <div className=" flex gap-1 absolute">
              <div
                className=" h-3 w-3 rounded-full bg-red-400 flex items-center justify-center cursor-default hover:bg-red-500 transition-colors hover:shadow"
            ...
            <div className=" w-full flex gap-1 justify-end">
              ...
              <Button
                onClick={() => navButtons.func({ view: "grid" })}
                isActive={navButtons.state === "grid"}
                isActiveClassName=" bg-white hover:bg-none"
              >
                <Icons.Grid4 className=" h-5 w-5" />
              </Button>
              <Button
                onClick={() => navButtons.func({ view: "row" })}
                isActive={navButtons.state === "row"}
                isActiveClassName=" bg-white hover:bg-none"
              >

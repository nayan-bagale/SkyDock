...
import { createAccessToken, createRefreshToken, decodeToken, verifyToken } from '../utils/token';
...
class AuthController {
  ...
  private constructor() {
    passport.use(
      new GoogleStrategy(googleOAuthConfig, async (_accessToken, _refreshToken, profile, done) => {
        try {
  ...
  async register(req: Request, res: Response) {
    ...
    if (!emailValidation(email).valid) {
      return res.status(UNAUTHORIED).json({ message: emailValidation(email).message });
    }
    ...
    if (!passwordValidation(password).valid) {
      return res.status(UNAUTHORIED).json({ message: passwordValidation(password).message });
    }
  ...
  async login(req: Request, res: Response) {
    ...
    if (!user.password) {
      return res.status(UNAUTHORIED).json({ message: 'Please sign in with google' });
    }
  ...
  async refresh(req: Request, res: Response) {
    ...
    if (!refreshToken) return res.status(UNAUTHORIED).json({ message: messages.UNAUTHORIED });
  ...
  async changePassword(req: Request, res: Response) {
    ...
    if (!passwordValidation(newPassword).valid) {
      return res.status(BADREQUEST).json({ message: passwordValidation(newPassword).message });
    }
    let user;
  ...
  async setPassword(req: Request, res: Response) {
    ...
    const userId = req.userInfo?.id as string;
    if (!password) return res.status(BADREQUEST).json({ message: 'All fields are required' });

    if (!passwordValidation(password).valid) {
      return res.status(BADREQUEST).json({ message: passwordValidation(password).message });
    }
  ...
  async verifyEmail(req: Request, res: Response) {
    ...
    try {
      ...
      if (isEmailVerified?.verified) {
        return res.status(FORBIDDEN).json({ message: 'Email already verified' });
      }
  ...
  async sendVerificationEmail(req: Request, res: Response) {
    ...
    if (user.verified) {
      return res.status(UNAUTHORIED).json({ message: 'Email already verified' });
    }
  ...
  async sendOtpToEmail(req: Request, res: Response) {
    const email = req.query.email as string;
    if (!email) {
      return res.status(UNPROCESSABLE_ENTITY).json({ message: 'Invalid email' });
    }
    ...
    if (!user) {
      return res.status(UNPROCESSABLE_ENTITY).json({ message: 'Email does not exist' });
    }

    if (!user.verified) {
      return res.status(UNPROCESSABLE_ENTITY).json({ message: 'Email is not verified' });
    }

    if (!user.password) {
      return res.status(UNPROCESSABLE_ENTITY).json({ message: 'Please sign in with google' });
    }
  ...
  async forgotPasswordReset(req: Request, res: Response) {
    ...
    if (!email || !password)
      return res.status(UNPROCESSABLE_ENTITY).json({ message: 'All fields are required' });

    if (!emailValidation(email).valid) {
      return res.status(UNPROCESSABLE_ENTITY).json({ message: emailValidation(email).message });
    }
  ...
  async OAuthGoogleCallback(req: Request, res: Response, next: NextFunction) {
    passport.authenticate('google', { session: false }, (err, user, info) => {
      ...
      return res.redirect(`${process.env.CLIENT_URL}/oauth?accessToken=${accessToken}`);

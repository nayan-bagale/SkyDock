...
export type DraggedFilesT = Pick<
...
export const DragDropWrapper = forwardRef<HTMLDivElement, DragDropWrapperProps>(
  (
    { children, handleExternalfiles, handleInternalFiles, disableDrag },
    ref,
  ) => {
    ...
    return (
      <div className="p-1 w-full h-full" ref={ref}>
        <div
          className={cn(
            "w-full h-full relative rounded-2xl p-0.5 transition-colors duration-300 ",
            dragging &&
              " z-50 border-2 border-dashed backdrop-blur bg-white/20 border-gray-400",
          )}
        ...
        >
          ...
          {dragging && (
            <div className="bottom-6 left-1/2 absolute flex flex-col items-center gap-2 w-fit text-xs -translate-x-1/2 transform">
              <motion.div
                className="bg-white shadow p-1 rounded-full w-fit"
                animate={{ y: [0, -10, 0] }}

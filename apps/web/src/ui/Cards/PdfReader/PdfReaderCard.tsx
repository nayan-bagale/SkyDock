...
const PdfReaderCard = forwardRef<HTMLDivElement, PdfReaderCardProps>(
    ({ style, theme, onMouseDown, action, children, onMouseDownCard, className, title, onContextMenu }, ref) => {
        return (
            <motion.div
                className={cn("text-black bg-white resize shadow absolute min-w-[30rem] w-[30rem] max-w-[45rem] h-[28rem] min-h-[30rem] max-h-[40rem] backdrop-blur rounded-xl overflow-hidden flex flex-col",
                    // theme?.color,
                    className
                )}
                ref={ref}

...
const ControlCenter = () => {
  ...
  const notification = useAppSelector(
    (state) => state.controlCenter.notifications,
  );
      ...
      <AnimatePresence>
        {show && (
          <motion.div
            ...
            ref={ref}
            className={cn(
              " absolute z-10 top-7 h-40 w-28 bg-white/50 backdrop-blur p-2 text-xs min-w-[18rem] gap-1 shadow-md flex flex-col rounded-xl right-2/4 translate-x-2/4",
            )}
          >
            <AnimatePresence>
              {notification.map((item, index) => (
                ...
                >
                  <div className=" z-10 mb-1 flex items-center gap-1 px-1.5 justify-between w-full">
                    ...
                    <Button
                      size={"icon"}
                      onClick={() => item.cancelUpload && item.cancelUpload()}
                    >

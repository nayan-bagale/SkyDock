...
const MusicPlayerCard = forwardRef<HTMLDivElement, MusicPlayerCardProps>(
  (
    ...
    return (
      <motion.div
        className={cn(
          "text-black resize shadow absolute min-w-[20rem] max-w-[45rem] min-h-[30rem] max-h-[30rem] backdrop-blur rounded-xl overflow-hidden",
          // theme?.color,
      ...
      >
        <div
          className={cn(
            "relative flex justify-between items-center bg-white/60 backdrop-blur shadow px-2 py-3 w-full",
            theme?.color,
          )}

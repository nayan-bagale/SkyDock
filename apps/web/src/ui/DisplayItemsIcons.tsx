    ...
    <>
      <AnimatePresence>
        {view === "grid" && (
          ...
          >
            ...
            <p
              className={cn(
                "text-[14px] relative cursor-default text-center select-none ",
                "truncate h-7 w-[11ch] overflow-hidden",
              )}
            >
              {item.name}
            </p>
            {!item.isFolder &&
              item?.state &&
              item?.state?.currentState === "uploding" && (
                <>
                  <div className="bottom-0 right-0 overflow-hidden absolute bg-black/10 rounded-md w-full h-full">
                    <motion.div
                      className="bg-black/40"
                      initial={{ height: "100%" }}
                      animate={{ height: `${100 - item.state.progress}%` }}
                    />
                  </div>
                  <div className="absolute flex justify-center items-center bg-black/50 shadow-sm backdrop-blur p-2 rounded-full font-bold text-white">
                    {item.state.progress}%
                  </div>
                </>
              )}
      ...
      <AnimatePresence>
        {view === "row" && (
          <motion.div
            className={cn(
              "hover:bg-gray-400/40 text-xs relative w-full flex justify-start items-center p-1 gap-2 focus:bg-gray-400/40 rounded-md",
              isOver && "bg-gray-400/10 border",
              className,
            )}
            id={item.id}
          ...
          >
            <Icon className="w-6" />
            <p className="text-center cursor-default select-none">
              {item.name}
            </p>

...
const ChangeName = memo(({ closeModal }: CommonProps) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      closeModal();
      showToast(res.message ?? "Name changed successfully", "success");
    } catch (e: any) {
      console.error(e);
  ...
  return (
    ...
    >
      ...
      <Form onSubmit={handleSubmit} className=" py-2 w-full">
        <label className=" text-sm self-start" htmlFor="fname">
          First Name
        </label>
        <Input
          id="fname"
        ...
        <label className=" text-sm self-start" htmlFor="lname">
          Last Name
        </label>
        <Input
          id="lname"
...
const ChangePassword = memo(({ closeModal }: CommonProps) => {
  ...
  return (
    ...
    >
      ...
      <Form onSubmit={handleSubmit} className=" py-2 w-full">
        <label className=" text-sm self-start" htmlFor="current-password">
          Current Password
        </label>
        <InputPassword
          id="current-password"
        ...
        {formError.currentPassword && (
          <ErrorMessage>{formError.currentPassword}</ErrorMessage>
        )}
        <label className="text-sm self-start" htmlFor="new-password">
          New Password
        </label>
        <InputPassword
          id="new-password"
        ...
        {formError.newPassword && (
          <ErrorMessage>{formError.newPassword}</ErrorMessage>
        )}
        <label className="text-sm self-start" htmlFor="confirm-new-password">
          Confirm Password
        </label>
        <InputPassword
...
const SetPassword = memo(({ closeModal }: CommonProps) => {
  ...
  return (
    ...
    >
      ...
      <Form onSubmit={handleSubmit} className=" py-2 w-full">
        <label className="text-sm self-start" htmlFor="password">
          Password
        </label>
        <InputPassword
          id="password"
  ...
  return (
    ...
    >
      ...
      <Form onSubmit={() => {}} className=" py-2 w-full">
        <label className=" text-sm self-start" htmlFor="new-email">
          New Email
        </label>
        <Input
          id="new-email"
        ...
        <InputPassword
          ...
          className=" placeholder:text-sm placeholder:text-gray-400"
        />
        <Button intent={"action"} className="py-1 mt-2" size={"medium"}>
          Change Email
        </Button>
      </Form>
    </motion.div>
...
const DeleteAccount = ({ closeModal }: CommonProps) => {
  return (
    ...
    >
      ...
      <p className="text-sm text-red-500 font-semibold py-2 text-center">
        Are you sure you want to delete your account? All your files and data
      ...
      <Form onSubmit={() => {}} className=" py-2 w-full">
        ...
        <Button
          intent={"action"}
          className="py-1 mt-2 bg-red-500"
          size={"medium"}
        >
...
const Account = () => {
  ...
  const Component = options.find((option) => option.id === openModal)
    ?.Component as
    | typeof ChangeName
          ...
          <div className="space-y-4">
            {options.map(({ id, name }, index) => {
              if (id === "03") {
                return (
                  <Button
                    className="flex gap-3 bg-red-500  w-full max-w-96 shadow backdrop-blur px-3 py-3 rounded-md"
                    key={id}
                    intent={"action"}
              ...
              return (
                <Button
                  className="flex gap-3 bg-white w-full max-w-96 shadow backdrop-blur px-3 py-3 rounded-md"
                  key={id}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{
                    opacity: 1,

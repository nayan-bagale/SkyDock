...
const CameraApp = () => {
  ...
  const cameraState = useAppSelector(
    (state) => state.skydock.browserApis.camera,
  );
  ...
  const availabeStorage = useAppSelector((state) => {
    const totalStorage =
      (state.auth.user?.plan.storageLimit ?? 0) - StorageLimit.MB_10;
  ...
  const startRecording = useCallback(() => {
    ...
    mediaRecorder.ondataavailable = (event) => {
      if (event.data.size > 0) {
        ...
        console.log(
          `Recorded chunk size: ${event.data.size}, Total size: ${totalSize}, Available storage: ${availabeStorage}`,
        );
    ...
    mediaRecorder.onstop = async () => {
      ...
      if (totalSize >= availabeStorage) {
        ...
        if (recordedChunks.length === 0) {
          ...
          setTimeout(() => {
            showToast(
              "Max storage size reached, stopping recording...",
              "error",
            );
          }, 1000);
        }
      ...
      <div className="absolute gap-4 inset-0 flex flex-col items-center justify-center bg-black bg-opacity-50 z-10">
        ...
        <div className="inset-0 flex flex-col gap-2 items-center justify-center z-20">
          ...
          <Button
            ...
            size="sm"
            intent={"cta"}
            className="bg-gradient-to-r from-lime-400 to-lime-500 text-white"
          >
            Upgrade Plan
          </Button>
        </div>
          ...
          <div className="absolute gap-4 inset-0 flex flex-col items-center justify-center bg-black bg-opacity-50 z-10">
            ...
            <div className="inset-0 flex flex-col gap-2 items-center justify-center z-20">
              {cameraState.permission === "Prompt" ? (
                ...
                <p className="text-white text-lg font-semibold">
                  Enable camera permission from browser settings
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex w-full absolute bottom-9 justify-center gap-4">
            {!isRecording && (
              <Button
                ...
                disabled={disabledButtons || isRecording}
            ...
            {!isRecording ? (
              ...
              <Button
                className="p-2 px-3 bg-red-600 text-white rounded-full"
                disabled={disabledButtons}

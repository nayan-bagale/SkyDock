...
const Explorer = () => {
  ...
  const isFileActionModalOn = useAppSelector(
    (state) => state.explorer.actions.isFileActionModalOn,
  );
  const explorerItems = useAppSelector((state) => state.explorer.explorerItems);
  const currentFolder = useAppSelector(
    (state) => state.explorer.explorerItems[state.explorer.currentFolder],
  ) as FolderT;
  const backStack = useAppSelector((state) => state.explorer.backStack);
  const backStackFoldersName = Object.values(
    useAppSelector((state) => state.explorer.explorerItems),
  )
    .filter((item) => backStack.includes(item.id))
    .map((item) => ({ id: item.id, name: item.name }));
  const forwardStack = useAppSelector((state) => state.explorer.forwardStack);
  const isFocused = useAppSelector(
    (state) => state.apps.focusedApp === AppsT.Explorer,
  );
  ...
  const handleEmptyTrash = async () => {
    const arrayItems = getNestedFolderItemsId("trash", []).filter(
      (item: any) => !ExplorerTabs.includes(item),
    );
    try {
  ...
  const settings = {
    view: {
      func: (v: ExplorerT["settings"]["view"]) => {

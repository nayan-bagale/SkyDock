...
const Signup: FC<SignupProps> = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!emailValidation(email).valid) {
      setFormError((prev) => ({
        ...prev,
        email: emailValidation(email).message,
      }));
      return;
    }
    ...
    if (!passwordValidation(password).valid) {
      setFormError((prev) => ({
        ...prev,
        password: passwordValidation(password).message,
      }));
      return;
    }
    ...
    if (!passwordValidation(confirmPassword).valid) {
      setFormError((prev) => ({
        ...prev,
        confirmPassword: passwordValidation(confirmPassword).message,
      }));
      return;
    }
  ...
  return (
    <AuthCard>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Input
          className={cn(formError.email && "border-red-500")}
          id="email"

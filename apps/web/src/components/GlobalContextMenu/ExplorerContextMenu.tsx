...
const ExplorerContextMenu = ({
  ...
  const isTrashTab = useAppSelector(
  ...
  if (!targetItem) {
    const hasClipboardItems =
      clipboardItems.items.length > 0 && clipboardItems.operation !== null;
    ...
    return (
      <>
        ...
        <Button
          size={"menu"}
          onClick={() => handleGenerateEmptyFile(currentFolder, "txt")}
        >
          <div>New Text</div>
  ...
  return (
    <>
      {targetItem.isFolder && (
        <Button
          size={"menu"}
          onClick={() => handleOpen(targetItem, "explorer")}
        >
          <div>Open</div>
        </Button>
      )}
      {!targetItem.isFolder && (
        <>
          {SupportedMimeTypeCheck(targetItem.details.type) && (
            <Button size={"menu"} onClick={() => handleOpen(targetItem)}>
              <div>Open</div>
            </Button>
          )}

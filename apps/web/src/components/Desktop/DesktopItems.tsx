...
const DesktopItems = () => {
      ...
      if (event.ctrlKey || event.metaKey) {
        ...
        const lastSelectedIndex =
          selectedItems.length > 0
            ? files.findIndex((i) => selectedItems.includes(i.id))
            : -1;
  ...
  const onKeydown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>, item: ExplorerItemT) => {},
    [],
  );
  ...
  return (
    <div
      className={
        "relative max-h-[90vh] inline-flex flex-col gap-2 items-start justify-start flex-wrap p-2"
      }
    >
